...
/*
 * Kassepåminnelsen: två tokens med olika etiketter, båda 30 dagar.
 *
 * `aterstall` bär PaymentIntenten så varukorgen kan läggas tillbaka utan att
 * kunden bygger om den. `avreg` bär adressen så en avregistrering kan skrivas
 * utan inloggning. Egna etiketter betyder att ingen av dem duger som session
 * eller som omdömeslänk, även om någon får tag på den.
 */
const PAMINNELSE_SEKUNDER = 30 * 24 * 60 * 60

export function skapaAterstallToken(piId) {
  return signera(
    { pi: piId, exp: Math.floor(Date.now() / 1000) + PAMINNELSE_SEKUNDER },
    "batteriproffs-aterstall-v1"
  )
}

export function lasAterstallToken(token) {
  return verifiera(token, "batteriproffs-aterstall-v1")?.pi || null
}

export function skapaAvregToken(epost) {
  return signera(
    { epost: String(epost).trim().toLowerCase(), exp: Math.floor(Date.now() / 1000) + PAMINNELSE_SEKUNDER },
    "batteriproffs-avreg-v1"
  )
}

export function lasAvregToken(token) {
  return verifiera(token, "batteriproffs-avreg-v1")?.epost || null
}
