...
export function parsaItems(raw) {
  return readOrderItems({ items: raw })
}
export { readOrderItems } from "./order-items"
...
export function normaliseraOrder(pi) {
  ...
  const { rader, strukna, incomplete } = readOrderItems(meta)
  ...
  return {
    ...
    reservationsalder:
      pi.status === "requires_capture"
        ...
        : null,
    items: rader.map(r => ({ ...r, name: r.name || getProductBySlug(r.slug)?.name || r.slug || "Artikel" })),
...
const hamtaRiktigaOrdrarCachat = unstable_cache(
  async () => {
    if (process.env.BP_REVIEW_PREVIEW === "1") return []
    const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, { maxNetworkRetries: 4 })
    const svar = await medBackoff(() => stripe.paymentIntents.list({ limit: 100 }))
    return svar.data
      .filter(
        (pi) =>
      ...
      .map((pi) => ({ datum: pi.created, rader: readOrderItems(pi.metadata).rader }))
  },
  ["riktiga-ordrar"],
