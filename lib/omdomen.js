...
// maxNetworkRetries: bygget förrenderar 100+ sidor parallellt och slog i
// Stripes ratelimit — SDK:n backar och försöker om i stället för att fälla.
...
/**
 * Stripes SDK backar INTE på 429 (bara nätfel, 409 och 5xx), och bygget
 * regenererar 100+ sidor samtidigt — uppmätt "Request rate limit exceeded"
 * i layouten 2026-09-02 trots maxNetworkRetries. Egen backoff på just 429.
 */
async function medBackoff(fn, forsok = 6) {
  for (let i = 0; ; i++) {
    try {
      return await fn()
    } catch (err) {
...
/**
 * Alla godkända omdömen, nyast först, cachade med taggen "omdomen".
 *
 * Läses i root-layouten så att varje produktkort på sajten kan visa stjärnor
 * utan att varje sida gör en egen Stripe-genomgång. Adminens godkännande
 * kör revalidateTag("omdomen") + revalidatePath("/", "layout"), så ett nytt
 * omdöme syns överallt direkt — inte när timcachen råkar gå ut.
 */
const cachadLasning = unstable_cache(
  async () => {
    const alla = await hamtaAllaOmdomen()
    return alla.filter((o) => o.status === GODKANT).sort((a, b) => b.datum - a.datum)
  },
  ["godkanda-omdomen"],
  { revalidate: 3600, tags: ["omdomen"] }
)

// Samtidiga anrop i samma process delar EN pågående läsning. Vid bygget
// startar flera sidor samtidigt innan cachen hunnit skrivas; utan det här
// blev det ett Stripe-anrop per sida och ratelimit.
let pagaende = null
export function hamtaGodkandaCachat() {
  if (process.env.BP_REVIEW_PREVIEW === "1") return Promise.resolve([])
  if (!pagaende) {
    pagaende = cachadLasning().finally(() => {
      setTimeout(() => {
        pagaende = null
      }, 5000)
    })
...
/** { slug: { antal, snitt } } — underlag för stjärnor på produktkorten. */
export function betygPerProdukt(godkanda) {
  const per = {}
  for (const o of godkanda) (per[o.slug] ??= []).push(o)
  return Object.fromEntries(Object.entries(per).map(([slug, lista]) => [slug, sammanfatta(lista)]))
}
