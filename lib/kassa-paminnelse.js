...
import { readOrderItems } from "./orders"
...
function radlista(pi) {
  const { rader } = readOrderItems(pi.metadata)
...
function knapp(url, text) {
  return `<p style="margin:0 0 22px;">
    <a href="${url}" class="bp-knapp" style="display:inline-block;background:#0B1D3A;color:#fff;text-decoration:none;padding:13px 24px;border-radius:10px;font-weight:700;font-size:15px;">
      ${escape(text)}
    </a>
  </p>`
}
