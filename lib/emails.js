...
// Isolated payment-test overlay. Never deployed to production.
const sendRealEmail = resend.emails.send.bind(resend.emails)
resend.emails.send = async (message, options) => {
  if (process.env.BP_PAYMENT_TEST !== "1" || !process.env.STRIPE_SECRET_KEY?.startsWith("sk_test_")) throw new Error("Test mail environment closed")
  const recipients = [message.to, message.cc, message.bcc].flat().filter(Boolean)
  if (!recipients.length || recipients.some((email) => email !== "joel@stoltmarketing.se")) throw new Error("Recipient is outside the payment test allowlist")
  return sendRealEmail({...message, subject: "[TEST] " + message.subject}, options)
}
  ...
  <tr><td align="center">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" class="bp-kort" style="max-width:560px;width:100%;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #E5E7EB;">
      <tr><td class="bp-header" style="background:#0B1D3A;padding:20px 24px;">
        <!--
          Loggan sitter i egna tabellceller med valign="middle" i stället för
          inline-block med ett negativt vertical-align. Den gamla lösningen
          sköt blixtrutan 7px ned med en magisk siffra, och den siffran stämde
          bara vid exakt 18px text i exakt ett typsnitt. I mejlklienter som
          byter typsnitt hamnade rutan snett. Tabellceller centrerar på riktigt,
          även i Outlook.
        -->
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
          <td valign="middle" width="26" style="width:26px;padding:0 8px 0 0;">
            <div style="width:26px;height:26px;background:#FDB813;border-radius:6px;text-align:center;line-height:26px;font-size:14px;color:#0A1628;">&#9889;</div>
          </td>
          <td valign="middle" class="bp-logo" style="font-family:system-ui,sans-serif;font-size:18px;font-weight:800;color:#ffffff;letter-spacing:-0.01em;white-space:nowrap;">Batteri<span class="bp-accent" style="color:#FDB813;">proffs</span></td>
          <td valign="middle" align="right" class="bp-domain" style="font-size:12px;color:#8A93A6;">www.batteriproffs.se</td>
        </tr></table>
      </td></tr>
