...
// 0 = skicka direkt (utan schemaläggning) — används för testutskick.
// Tom/osatt variabel faller alltid tillbaka på 5, aldrig på 0 av misstag.
const rawDelay = process.env.REVIEW_EMAIL_DELAY_DAYS
const DELAY_DAYS =
  rawDelay?.trim() && Number.isFinite(Number(rawDelay)) ? Number(rawDelay) : 5
...
export async function scheduleReviewEmail({
  ...
  const result = await resend.emails.send({
    ...
    to,
    replyTo,
    ...(DELAY_DAYS > 0 ? { scheduledAt: `in ${DELAY_DAYS} days` } : {}),
    subject: firstName
    ...
    html: emailLayout({
      ...
      body: buildReviewBody({
        firstName,
        orderId,
        reviewUrl: reviewUrlFor(piId, to),
      }),
    }),
  })
  if (result?.error || !result?.data?.id)
    throw new Error("Mejltjänsten kunde inte schemalägga omdömesmejlet")
  return result
}
