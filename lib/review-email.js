...
function buildReviewBody({ firstName, orderId, reviewUrl }) {
  ...
  return `
    ...
    <p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:#374151;">
      Om du har en stund över skulle det betyda mycket om du la en kort
      ärlig recension på Google. Det hjälper andra kunder att hitta oss
      och ger oss feedback på hur vi kan bli bättre.
    </p>
    ...
    <p style="margin:24px 0 0;font-size:13px;line-height:1.6;color:#6B7280;">
      ...
      <a href="tel:+46735546968" style="color:#0B1D3A;font-weight:600;text-decoration:none;">073-554 69 68</a>
...
export async function scheduleReviewEmail({ to, fullName, orderId, replyTo }) {
  if (!REVIEW_URL || !to || !orderId) return null
  ...
  return resend.emails.send({
    ...
    html: emailLayout({
      title: `Lämna en recension på Google`,
      preheader: `Tack för din order — vill du dela din upplevelse?`,
      body: buildReviewBody({ firstName, orderId, reviewUrl: REVIEW_URL }),
    }),
  })
}
